...
To use your topic for regularization, you need to remap the `input_regularization_pose_topic` with your topic in `autoware_ndt_scan_matcher.launch.xml`.
...
Autoware supports dynamic map loading feature for `autoware_ndt_scan_matcher`. Using this feature, NDT dynamically requests for the surrounding pointcloud map to `pointcloud_map_loader`, and then receive and preprocess the map in an online fashion.

Using the feature, `autoware_ndt_scan_matcher` can theoretically handle any large size maps in terms of memory usage. (Note that it is still possible that there exists a limitation due to other factors, e.g. floating-point error)
...
To use dynamic map loading feature for `autoware_ndt_scan_matcher`, you also need to appropriately configure some other settings outside of this node.
