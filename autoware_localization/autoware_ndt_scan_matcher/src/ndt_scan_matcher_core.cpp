...
#include "autoware_ndt_scan_matcher/ndt_scan_matcher_core.hpp"

#include "autoware_ndt_scan_matcher/matrix_type.hpp"
#include "autoware_ndt_scan_matcher/particle.hpp"
#include "autoware_ndt_scan_matcher/pose_array_interpolator.hpp"
#include "autoware_ndt_scan_matcher/util_func.hpp"
...
: Node("autoware_ndt_scan_matcher"),
  ...
  z_margin_for_ground_removal_(declare_parameter("z_margin_for_ground_removal", 0.8))
{
  ...
  sensor_points_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    ...
    // "points_raw",rclcpp::QoS(rclcpp::KeepLast(points_queue_size)).durability_volatile().reliable(),
...
void NDTScanMatcher::timer_diagnostic()
{
  ...
  while (rclcpp::ok()) {
    ...
    diag_status_msg.name = "autoware_ndt_scan_matcher";
  ...
  const geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr initial_pose_msg_ptr)
{
  ...
  // RCLCPP_WARN_STREAM_THROTTLE(this->get_logger(), *this->get_clock(), 1, "Received initial_pose_msg_ptr_array_ ... ");
...
{
  // RCLCPP_WARN_STREAM_THROTTLE(this->get_logger(), *this->get_clock(), 1, "Received sensor points ... ");
  // mutex ndt_ptr_
  ...
  // ndt_scan_matcher 没有报错但是也没有发布tf和pose ，is_activated_ 为false导致 
  // 手动发送 ： ros2 service call /trigger_node std_srvs/srv/SetBool "data: true"
  if (!is_activated_) return;
  ...
  converge (be careful it's 'greater than' instead of 'greater equal than'.)
     https://github.com/tier4/autoware.iv/blob/2323e5baa0b680d43a9219f5fb3b7a11dd9edc82/localization/pose_estimator/autoware_ndt_scan_matcher/ndt_omp/include/ndt_omp/ndt_omp_impl.hpp#L212
  2. iterate iteration count when end of gradient descent function.
     https://github.com/tier4/autoware.iv/blob/2323e5baa0b680d43a9219f5fb3b7a11dd9edc82/localization/pose_estimator/autoware_ndt_scan_matcher/ndt_omp/include/ndt_omp/ndt_omp_impl.hpp#L217
