...
#include "autoware_gyro_odometer/gyro_odometer_core.hpp"
...
/*
 * This test checks if twist is published from autoware_gyro_odometer
 */
...
// IMU & Velocity test
// Verify that the autoware_gyro_odometer successfully publishes the fused twist message when both IMU and
...
TEST(GyroOdometer, TestGyroOdometerWithImuAndVelocity)
{
  ...
  // autoware_gyro_odometer receives IMU and velocity, and publishes the fused twist data.
...
// IMU-only test
// Verify that the autoware_gyro_odometer does NOT publish any outputs when only IMU is provided
TEST(GyroOdometer, TestGyroOdometerImuOnly)
{
  ...
  // autoware_gyro_odometer receives IMU
  spinSome(gyro_odometer_node);

  // validator node waits for the output fused twist from autoware_gyro_odometer
  spinSome(gyro_odometer_validator_node);
